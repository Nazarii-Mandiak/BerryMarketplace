...
Phase 5 adds a marketplace-aware chat assistant: a user has one or more conversations, each holding a sequence of messages, and the assistant runs a tool-calling loop server-side — able to search listings, look at a user's reservations, and similar marketplace actions — rather than just answering from the prompt alone. Task 19 lays the persistence foundation for that: per-user `ChatConversation`/`ChatMessage` entities and `GET/POST /api/chat/conversations` + `GET /api/chat/conversations/{id}/messages`, with no AI or tool-calling logic yet (that's Tasks 20–21). Ownership is enforced the same way `ReservationsService`/`ListingsService` already scope data to a user: every read filters by the authenticated user's ID, and `ChatService.GetMessagesAsync` returns `null` (not an empty list) when the conversation isn't owned by the caller, which `ChatEndpoints` turns into a `404` rather than leaking a `200` with an empty body that would let a client distinguish "empty conversation" from "someone else's conversation" by a different signal.

Conversation history is persisted as plain text turns — `Role` (`"user"` or `"assistant"`) and `Content` — replayed back to Claude as the conversation's message history on each turn. Tool calls and tool results that occur *within* an assistant turn (Tasks 20–21) are not persisted as their own rows; only the final text content of each turn is stored. This is a deliberate, documented simplification: it keeps the schema and the replay logic in this phase simple (two entities, two tables, no polymorphic message-content model), at the cost that a resumed conversation replays as if the assistant answered directly, without the tool traffic that actually produced the answer. If a future task needs the assistant to reason about *its own prior tool calls* across turns — rather than just the prior turns' final text — this is the structural gap that would need to be revisited (e.g. a `ToolCalls` json column on `ChatMessage`, or a separate table for tool-call/tool-result pairs scoped to a message).

`AnthropicChatAgentModel` calls `claude-haiku-4-5-20251001` rather than `claude-opus-5`. Unlike the listing assistant and tasting notes (one-shot, latency-tolerant, quality-sensitive), chat is an interactive, multi-turn, tool-calling loop where every user message triggers at least one round trip — a cheaper, faster model keeps the loop responsive without materially hurting quality on the bounded tool set (search, listing lookup, availability, reservations) this assistant operates over.
