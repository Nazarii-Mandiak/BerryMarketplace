...
**DLQ topology (Task 8):** the AI worker declares its own queue, `ai-enrichment`, bound to `berry.events` with routing key `listing.created`, with `x-dead-letter-exchange` set to a fanout exchange `berry.events.dlx`; a bound queue `ai-enrichment.dead` collects anything the worker `Nack`s (requeue: false) after a processing failure, so a bad message is quarantined for inspection instead of blocking or endlessly retrying the queue.
